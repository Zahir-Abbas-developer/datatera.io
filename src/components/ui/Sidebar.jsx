...
import {
  Menu, ChevronDown, ChevronUp, MoreVertical, Plus, X, Search, User, LogOut, ExternalLink, Star, Pencil, Copy, EyeOff, Trash2, ChevronLeft, ChevronRight
} from 'lucide-react';
...
const Sidebar = ({
  ...
  const handleMenuAction = (action, item) => {
    switch (action) {
      case 'edit':
        ...
        setEditingItem({
          name: item,
          description: '' // You can add a description field to your transformations array if needed
        });
        setShowModal(true);
        break;
      case 'copyWithoutData':
        // Handle copy without data
        break;
      case 'copyWithData':
        // Handle copy with data
        break;
      case 'unsubscribe':
        // Handle unsubscribe
        break;
  ...
  return (
    <aside
      className={`fixed top-0 left-0 h-screen bg-[#1E1E1E] text-white flex flex-col border-r border-[#232323] shadow-lg transition-all duration-200 ${
    ...
    >
      ...
      <button
        ...
        className="absolute  -right-4 z-20 bg-[#232323] text-white rounded-full shadow p-1 border border-[#232323] hover:bg-[#333] transition"
        style={{ width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center' ,backgroundClip:"black" ,"marginTop":"14px"}}
...
{!isCollapsed && <div className="px-1 pt-1 pb-1">
            ...
            {transformations.map((item, idx) => (
              <li key={idx} className="relative w-full">
                <div
                  ...
                  title={isCollapsed ? item : undefined} // Only show tooltip when collapsed
                >
                    {/* onClick={() => onSelect(item)} */}
                  <div className="flex items-center gap-2" onClick={()=>navigate('/transformation-table')}>
                    <Tooltip>
...
>
  {item}
        ...
        {/* Bottom Section */}
        <div
  className={`flex-shrink-0 border-t border-[#232323] px-2 py-4 space-y-1 w-full ${
